...
/* The list interfaces are exported from the linux source codes */
...
#ifndef NDEBUG	
	/* 
	 * To improve our perfermance, we do not reset
	 * the node 
	 */
...
{
	struct list_head *n_first = entry->next;
	list->next = head->next;
	list->next->prev = list;
	list->prev = entry;
	entry->next = list;
 ...
 * @list: a n list to add all removed entries
